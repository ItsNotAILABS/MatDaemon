"""Specialized CUDA GEMM backend for MatDaemon.

This restores the original hand-written RawKernel backend as a properly named
module. It is intentionally separate from the package wrapper so CUDA-focused
users can inspect, benchmark, and optimize the kernel directly.
"""
...
try:  # pragma: no cover - only available on CUDA hosts
    import cupy as cp
...
class CUDABackend(BaseBackend):
    """CuPy RawKernel GEMM backend with shared-memory and register tiling."""
